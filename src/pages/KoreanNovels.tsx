...
const KoreanNovels = () => {
  ...
  const fetchNovels = useCallback(async (page = 1, limit = itemsPerPage) => {
    try {
      setLoading(true);
      const result = await getNovelsByFilter('language', 'korean', page, limit);
      ...
      toast({
        title: "Error",
        description: "Failed to load Korean novels.",
        variant: "destructive",
      });
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        title: "Refreshed",
        description: "Korean novels list has been updated.",
      });
    } catch (error) {
      toast({
