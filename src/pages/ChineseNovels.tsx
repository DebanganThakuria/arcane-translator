...
const ChineseNovels = () => {
  ...
  const fetchNovels = useCallback(async (page = 1, limit = itemsPerPage) => {
    try {
      setLoading(true);
      const result = await getNovelsByFilter("language", "chinese", page, limit);
      ...
      toast({
        title: "Error",
        description: "Failed to load Chinese novels.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        title: "Refreshed",
        description: "Chinese novels list has been updated.",
      });
    } catch (error) {
      toast({
        ...
        <div className="space-y-6">
          {loading ? (
            <div className="glass-card rounded-lg p-10 text-center">
              <p className="text-xl text-muted-foreground mb-4">Loading Chinese novels...</p>
            </div>
          ) : pagination.novels.length > 0 ? (
            <div className="glass-card rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Chinese Novels</h2>
            ...
            <div className="glass-card rounded-lg p-10 text-center">
              <p className="text-xl text-muted-foreground mb-4">No Chinese novels found</p>
            </div>
          )}
        </div>
      </div>
